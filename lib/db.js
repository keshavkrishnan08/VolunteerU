...
export async function signIn(email, password) {
  ...
  // The auth listener in auth.js flips session.authed and loads the profile,
  // but do it here too so the caller can navigate immediately.
  if (data.user) {
    update((s) => {
      s.session.authed = true;
      s.session.email = data.user.email || email;
      s.session.signedInAt = Date.now();
...
export async function signUp({ email, password, firstName, lastName }) {
  ...
  update((s) => {
    ...
    if (firstName) {
      ...
      s.account.name = `${firstName} ${lastName || ''}`.trim();
    }
    if (lastName) s.account.lastName = lastName;
    s.account.email = email;
    s.onboarding.completed = false;
    s.onboarding.step = 1;
    s.meta.installedAt = s.meta.installedAt || Date.now();
  });
  return data;
}
...
export function chartSeries(range) {
  ...
  return rows.map((r) => ({
    ...
    h: `${r.h} hrs`,
    raw: r.h,
    height: `${Math.round((r.h / max) * 100)}%`,
    bg: r.h === max ? '#C2603C' : '#E4CDC0',
  }));
}
...
export function searchOpportunities({ q = '', causes = [], windows = [], maxMiles = null, sort = 'match', savedOnly = false } = {}) {
  ...
  let rows = allOpportunities().filter((o) => {
    ...
    if (effMiles != null && o.distance > effMiles) return false;
