...
export function initAuth() {
  ...
  supabase.auth.onAuthStateChange((event, session) => {
    // A recovery link may land on any path if Supabase's redirect config isn't
    // exact. Wherever it fires, send the user to the reset form (the session is
    // already in storage, so /reset can set the new password against it).
    if (event === 'PASSWORD_RECOVERY' && typeof window !== 'undefined' && !window.location.pathname.startsWith('/reset')) {
