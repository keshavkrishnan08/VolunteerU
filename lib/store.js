...
export function update(fn, opts = {}) {
  ...
  state = { ...state };
  if (!opts.transient) {
    // Stamp every real edit so a stale cloud copy can never overwrite newer
    // local changes on the next load (see loadUserState). Loads/hydration pass
    // { transient: true } and must NOT bump this.
