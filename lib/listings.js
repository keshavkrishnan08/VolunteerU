...
export async function matchListings({ interest = '', causes = [], near = '', limit = 9 } = {}) {
  ...
  for (const c of (causes || [])) {
    for (const w of interestKeywords(c, 3)) if (!kws.includes(w)) kws.push(w);
  }
  ...
  const score = (l) => {
    ...
    const isRemote = l.org_type === 'team' || /remote|online|virtual/i.test(l.site || '');
    if (loc && !isRemote && ((l.site || '').toLowerCase().includes(loc) || (l.location || '').toLowerCase().includes(loc))) sc += 2; // near in-person
    if (isRemote) sc += 0.5; // remote is available to everyone
    return sc;
  };
